...
if df.empty:
    ...
    # สกัดชื่อเดือนภาษาไทยออกมาจากข้อความวันที่ (เช่น "16 พฤษภาคม 2569") เพื่อนำไปทำสถิติตามเดือน
        ...
        with col2:
            ...
            for num in df_clean['เลขท้าย 2 ตัว']:
                if len(num) == 2 and num.isdigit():
                    tens.append(int(num[0]))
                    units.append(int(num[1]))
        ...
        if not df_month.empty:
            ...
            with m_col2:
                ...
                if not df_m_first.empty:
                    last_2_first = df_m_first['รางวัลที่ 1'].apply(lambda x: x[-2:] if len(x)==6 and x.isdigit() else None).dropna()
    ...
    with tab3:
        ...
        with c2:
            ...
            if not df_clean.empty:
                ...
                cold_scores.sort(key=lambda x: x[1]) # เรียงตามงวดเก่าสุดที่เจอ
