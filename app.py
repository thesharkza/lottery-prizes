...
# ====================================================================
def make_horizontal_bar(labels, values, height=380, label_suffix="ครั้ง"):
    """กราฟแท่งแนวนอน Modern Minimal สำหรับ Top rankings"""
    # ไล่ shade จากเข้ม→อ่อน ตาม rank
    n = len(values)
    colors = []
    for i in range(n):
        ratio = i / max(n - 1, 1)
        # blend between primary (4f46e5) → primary-light (818cf8)
...
def make_digit_bars(digit_counts, position_name):
    """กราฟแท่งความถี่ตาม 0-9 สำหรับแต่ละหลัก"""
    digits = list(range(10))
    counts = [digit_counts.get(d, 0) for d in digits]
    max_count = max(counts) if counts else 1

    # highlight top 3
    sorted_counts = sorted(set(counts), reverse=True)[:3]
    colors = []
    for c in counts:
        if c == sorted_counts[0]:
            colors.append(COLOR_PRIMARY)
        elif len(sorted_counts) > 1 and c == sorted_counts[1]:
            colors.append("#818cf8")
        elif len(sorted_counts) > 2 and c == sorted_counts[2]:
    ...
    fig = go.Figure(go.Bar(
        ...
        text=counts,
        textposition="outside",
        textfont=dict(family="IBM Plex Sans Thai, sans-serif", size=11, color=COLOR_TEXT),
...
def make_monthly_trend(months_data, label="ความถี่"):
    """กราฟเส้นแสดงแนวโน้มรายเดือน (smooth area)"""
    fig = go.Figure(go.Scatter(
        x=list(months_data.keys()),
        y=list(months_data.values()),
        mode="lines+markers",
        line=dict(color=COLOR_PRIMARY, width=3, shape="spline", smoothing=1.0),
        marker=dict(size=8, color=COLOR_PRIMARY, line=dict(color="white", width=2)),
        fill="tozeroy",
...
def make_consensus_bar(consensus_data):
    """กราฟ Top 10 จากคะแนนโหวตรวม Ensemble"""
    labels = [x[0] for x in consensus_data]
    scores = [x[1] for x in consensus_data]
...
with tab1:
    ...
    if not flat_data.empty:
        ...
        with col1:
            ...
            st.markdown(f'<div class="chart-title">🔝 Top 15 {selected_cat}</div>', unsafe_allow_html=True)
            ...
            fig = make_horizontal_bar(top_15.index.tolist(), top_15.values.tolist(), height=480)
        ...
        with col2:
            ...
            st.dataframe(top_df, use_container_width=True, hide_index=True, height=430)
            st.markdown('</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if not month_flat_data.empty:
        ...
        with col1:
            ...
            fig = make_horizontal_bar(top_10_m.index.tolist(), top_10_m.values.tolist(), height=380)
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
            st.markdown('</div>', unsafe_allow_html=True)
        with col2:
            st.markdown('<div class="chart-card">', unsafe_allow_html=True)
            ...
            st.dataframe(m_top_df, use_container_width=True, hide_index=True, height=330)
            st.markdown('</div>', unsafe_allow_html=True)
