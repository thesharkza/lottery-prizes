...
def make_horizontal_bar(labels, values, height=380, label_suffix="ครั้ง"):
    """กราฟแท่งแนวนอน Modern Minimal สำหรับ Top rankings - ปรับให้ใช้พื้นที่เต็ม"""
    # กรองรายการที่มี value = 0 ออก (ไม่ควรขึ้นบ่อยใน top แต่กันไว้)
    filtered = [(l, v) for l, v in zip(labels, values) if v > 0]
    if not filtered:
        return go.Figure().update_layout(**get_minimal_layout(height=height))
    ...
    # ปรับช่องว่างให้แท่งกินพื้นที่มากขึ้น
    layout["bargap"] = 0.25
    # ขยาย range เพื่อให้ text outside ไม่โดนตัด
    max_v = max(values)
